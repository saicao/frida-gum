...
// static void on_first_block_exec(GumCpuContext *cpu_context, gpointer user_data);
// static void on_first_block_exec(GumCpuContext *cpu_context, gpointer user_data) {
...
void end_build_block(JsonBuilder *meta,GumAddress block_address,GumAddress size){
...
void transform(GumStalkerIterator *iterator, GumStalkerOutput *output,
  ...
  while (gum_stalker_iterator_next(iterator, &insn)) {
    ...
    is_last_in_block = cs_insn_group(capstone, insn, CS_GRP_JUMP) ||
                       cs_insn_group(capstone, insn, CS_GRP_RET)||cs_insn_group(capstone, insn, CS_GRP_CALL);
  ...
  //this should be write properly because we can write at least 2048 bytes
  if(!is_last_in_block){
          gum_arm64_writer_put_stp_reg_reg_reg_offset(
          cw, AArch64_REG_X27, AArch64_REG_LR, session_reg,
          G_STRUCT_OFFSET(ITraceSession, saved_regs), GUM_INDEX_SIGNED_OFFSET);
